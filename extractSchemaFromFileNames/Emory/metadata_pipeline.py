...
def populateMetadata(gc, collectionID=None, folderID=None, outputFailed=False):
    ...
    if os.path.exists("./Tissue Catalog - current.csv"):
        clinicalDF = pd.read_csv("./Tissue Catalog - current.csv", dtype=str)
        # clinicalDF.fillna("Not Specified", inplace=True)
    ...
    updateDict, failedDict = extractMetadataFromFileName(itemsToEvaluate, clinicalDF=clinicalDF)
...
def extractMetadataFromFileName(slideData, matchPatterns=adrcNamePatterns, referenceDoc=None, clinicalDF=None):
    ...
    for ID, data in slideData.items():
        ...
        if metadata:
            ...
            if clinicalDF is not None:
                filt = clinicalDF["Case Number"] == validatedNPData["npSchema"]["caseID"]

                if filt.any():
                    npClinical = (clinicalDF[filt].to_dict(orient="records"))[0]
                    validatedNPData["npClinical"] = {key: val for key, val in npClinical.items() if pd.notna(val)}
                else:
                    validatedNPData["npClinical"] = None
